    ...
    try:
        ...
        while remaining_questions:
            ...
            client_socket.sendall(
                (MOVE_CURSOR.format(1, 1) + wrapped_question + "\n").encode()
            )
            ...
            client_socket.sendall(
                MOVE_CURSOR.format(
                    len(wrapped_question.split("\n")) + len(question["choices"]) + 3, 1
                ).encode()
            )
            ...
            if answer == question["correct"]:
                ...
                response = "Incorrect, The Correct Option Was {}\n".format(
                    question["correct"]
                )
            ...
            if (
                remaining_questions
            ):  # Only prompt to continue if there are questions left
                client_socket.sendall("Do you want to continue? (yes/no): ".encode())
                continue_response = ""
                while not continue_response.endswith("\n"):
                    continue_response += client_socket.recv(1024).decode()
                ...
                if continue_response.startswith("n"):
                    ...
                    client_socket.sendall(
                        CLEAR_SCREEN.encode()
                    )  # Clear screen for potential message to type "yes" or "no"
                    client_socket.sendall('Please type "yes" or "no": '.encode())
            else:
                client_socket.sendall(
                    "No more questions left. Thank you for participating!\n".encode()
                )
                break
